...
```{r pfig.cap='two plots', fig.subcap= c('A figure', 'A table'), echo = F}
library(plotly)
library(RCAS)
...
peakMat <- fread(file = peaks, sep = '\t', header = F, quote = '', stringsAsFactors = F)
peakMat <- data.frame(peakMat)
...
p1 <- plot_ly(
    x = factor(df$feature, levels = df$feature),
    y = df$count,
    type = "bar",
    color = RColorBrewer::brewer.pal(length(df$count), 'Paired')
)
...
lncRNACount <- df$Count[which(df$`Gene type` == "lincRNA")]
knitr::kable(x = df, align = 'c') %>%  kableExtra::kable_styling(position = 'center', full_width = FALSE, stripe_color = 'black', latex_options = "bordered")
```
...
p1 <- ggplot(df, aes(x = values)) +
    geom_density(fill='#88C9ED', colour=NA, alpha=1) +
    geom_line(stat="density", colour = NA) + 
...
p <- ggplot2::ggplot(df, aes(x = bases, y = meanCoverage)) + 
      geom_ribbon(aes(fill = 'red', ymin = meanCoverage - standardError * 1.96, 
