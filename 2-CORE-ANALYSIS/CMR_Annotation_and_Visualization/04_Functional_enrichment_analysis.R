...
output <- "/home/DeepEA/galaxy/tools/2-CORE-ANALYSIS/CMR_Annotation_and_Visualization/"
...
Ensembl2Entrez <- function(species_name = "Zea mays", geneID = NULL, drop = TRUE){
  	load("/home/DeepEA/galaxy/tools/2-CORE-ANALYSIS/CMR_Annotation_and_Visualization/ensembl_species.RData")
...
entrezID <- Ensembl2Entrez(species_name = species,
...
if(type == "GO"){
	pdf(file = paste0(output, "GO.pdf"), width = 13, height = 13)
	options(digits = 3)
	...
	ego2@result$Description <- ego2@result$ID
	write.table(ego2@result, file = paste0(output, "GO_enrichment.txt"),
	...
	pdf(file = paste0(output, "KEGG.pdf"), height = 13, width = 13)
	ath <- search_kegg_organism(species, by = 'scientific_name')[1,]
	eKEGG <- enrichKEGG(gene = entrezID,
						organism = ath$kegg_code,
						pvalueCutoff = as.numeric(pvalueCutoff))
	write.table(eKEGG@result, file = paste0(output, "KEGG_enrichment.txt"),
				sep = '\t', quote = F, row.names = F, col.names = T)
