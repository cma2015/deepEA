...
write.fasta(sequences = AASeq, names = names(AASeq), file.out = paste0(mainDic, "AA.fasta"))
cmd <- paste0("sed -i 's/*//g' ", paste0(mainDic, "AA.fasta"))
system(cmd)
outHomo <- MHmakeRandomString()
write.table(duplicated_pairs, file = paste0(mainDic, outHomo, ".txt"), sep = "\t", quote = F,
            row.names = F, col.names = F)
outDir <- MHmakeRandomString()
cmd <- paste0("perl /home/DeepEA/galaxy/tools/DeepEA_software/ParaAT2.0/ParaAT.pl -h ",
              paste0(mainDic, outHomo, ".txt"), " -a ", paste0(mainDic, "AA.fasta"),
              " -n ", CDSDir, " -msa muscle  -f axt -o ",
              paste0(mainDic, outDir),
              " -p /home/DeepEA/galaxy/tools/DeepEA_software/ParaAT2.0/proc ")
system(cmd)
cmd <- paste0("cat ",paste0(mainDic, outDir), '/*axt > ', mainDic, outDir, ".axt")
system(cmd)
cmd <- paste0("/home/DeepEA/galaxy/tools/DeepEA_software/KaKs_Calculator -i ", paste0(mainDic, outDir, ".axt"), " -m YN -o ", paste0(mainDic, outDir, ".kaks"))
system(cmd)
